...
public sealed class EnergyRollupTests
{
    ...
    public async Task MeterReconciliation_RemovesCrossMappedImportAndExportRows()
    {
        ...
        try
        {
            var repository=new SqliteDashboardRepository(path); await repository.InitializeAsync(); var start=DateTimeOffset.Parse("2026-06-18T00:00:00Z");
    ...
    public async Task RawReadings_RebuildAccurateDailyAndMonthlyRollups()
    {
        ...
        try
        {
            var repository = new SqliteDashboardRepository(path); await repository.InitializeAsync(); var start = new DateTimeOffset(2026, 6, 18, 0, 0, 0, TimeSpan.Zero);
            var raw = new[]
            {
            ...
            await repository.UpsertOctopusRawReadingsAsync(raw); await repository.UpsertOctopusStandingChargesAsync([new(new(2026,6,18),"GAS","GAS-T",0.30m)]); await repository.SetSolarManualOverrideAsync(new(2026,6,18)); await repository.RebuildOctopusRollupsAsync();
