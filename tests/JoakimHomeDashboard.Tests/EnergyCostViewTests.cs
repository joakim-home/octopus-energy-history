...
public sealed class EnergyCostViewTests
{
    ...
    public async Task Rollups_CalculateImportCostExportIncomeGasCostAndNet()
    {
        ...
        try
        {
            ...
            await repository.UpsertOctopusStandingChargesAsync([new(DateOnly.FromDateTime(DateTime.Today),"GAS","GAS-A",0.30m)]);
            ...
            Assert.Equal(0.65m, dashboard.TodayCost.GasCost); Assert.Equal(1.10m, dashboard.TodayCost.NetCost);
            Assert.True(dashboard.TodayCost.ImportExact); Assert.True(dashboard.TodayCost.ExportExact); Assert.True(dashboard.TodayCost.GasExact);
        }
    ...
    public async Task HistoricalCostBackfill_UpdatesExistingIntervalAndPeakBreakdown()
    {
        ...
        try
        {
            var repository=new SqliteDashboardRepository(path); await repository.InitializeAsync(); var start=new DateTimeOffset(DateTime.Today.Year,DateTime.Today.Month,1,0,0,0,TimeSpan.Zero);
    ...
    [Fact]
    public async Task SelectedViewMode_PersistsBetweenPreferenceInstances()
    {
        var path = Path.Combine(Path.GetTempPath(), $"joakim-view-{Guid.NewGuid():N}.db");
        try
        {
            var repository = new SqliteDashboardRepository(path); await repository.InitializeAsync();
            Assert.Equal(AnalysisChartType.Columns,await new UserPreferences(repository).GetYearAnalysisChartTypeAsync());
            Assert.Equal(AnalysisChartType.Columns,await new UserPreferences(repository).GetMonthlyCostChartTypeAsync());
            Assert.Equal(CostColumnMode.StackedBreakdown,await new UserPreferences(repository).GetCostColumnModeAsync());
            await new UserPreferences(repository).SetEnergyViewModeAsync(EnergyViewMode.Both);
            Assert.Equal(EnergyViewMode.Both,await new UserPreferences(repository).GetEnergyViewModeAsync());
            await new UserPreferences(repository).SetYearAnalysisChartTypeAsync(AnalysisChartType.Columns);
            Assert.Equal(AnalysisChartType.Columns,await new UserPreferences(repository).GetYearAnalysisChartTypeAsync());
            await new UserPreferences(repository).SetMonthlyCostChartTypeAsync(AnalysisChartType.Line);
            await new UserPreferences(repository).SetCostColumnModeAsync(CostColumnMode.NetCost);
            Assert.Equal(AnalysisChartType.Line,await new UserPreferences(repository).GetMonthlyCostChartTypeAsync());
            Assert.Equal(CostColumnMode.NetCost,await new UserPreferences(repository).GetCostColumnModeAsync());
        }
        finally { if (File.Exists(path)) File.Delete(path); }
    }

    [Fact]
    public void EmptyCostSeries_DoesNotRequestDateAxisAndHoverModelFormatsExactPoint()
    {
        Assert.False(ChartPresentation.HasFiniteValues([double.NaN,double.NaN]));
        var text=ChartPresentation.FormatHover([new(new(2026,6,18),"Import",0.42m,"£",ImportRateBand.OffPeak)]);
        Assert.Contains("18 Jun 2026",text,StringComparison.Ordinal); Assert.Contains("Import (off-peak): £0.4200",text,StringComparison.Ordinal);
    }
}
