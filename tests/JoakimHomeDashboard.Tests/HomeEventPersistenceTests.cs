...
public sealed class HomeEventPersistenceTests
{
    ...
    public async Task Events_CanBeCreatedEditedAndDeleted()
    {
        ...
        try
        {
            var repository = new SqliteDashboardRepository(path); await repository.InitializeAsync(); await repository.SaveHomeEventAsync(new(0,new(2024,5,1),"Solar installed","Solar"));
